...
This file is part of ET: Legacy source code.
...
along with ET: Legacy source code; if not, write to the Free Software
...
/***********************************************************************
 * The following functions for predicting player positions have been
 * adopted from 'g_unlagged.c' which is part of the 'unlagged' system
 * created by Neil "haste" Toronto: http://www.ra.is/unlagged
 ***********************************************************************/
...
static int predict_slide_move(sharedEntity_t *ent, float frametime, trajectory_t *tr, vec3_t result) {
	...
	for (count = 0; count < NUMBUMPS; count++) {
		...
		// out along it, which fixes some epsilon issues with non-axial planes
		...
		// modify velocity so it parallels all of the
		// clip planes find a plane that it enters
		for (i = 0; i < numplanes; i++) {
...
int SV_CanSee(int player, int other) {
	...
	// changed during the move. This could introduce some errors.
	if (sv_wh_check_fov->integer > 0) {
